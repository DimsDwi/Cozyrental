...
<main>
  ...
  <section style="padding: 5rem 2rem 3rem; text-align: center; max-width: 650px; margin: 0 auto;">
    ...
    <h1 style="font-size: 3rem; font-weight: 900; letter-spacing: -1.5px; margin-bottom: 0.75rem;">
      Semua <span style="background: linear-gradient(135deg, #818cf8, #06b6d4); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">Armada Kami</span>
    </h1>
    <p style="color: var(--muted); font-size: 1.05rem;"><?php echo count($cars); ?> kendaraan tersedia &mdash; filter berdasarkan tipe, cari berdasarkan nama, atau urutkan berdasarkan harga.</p>
  </section>
  ...
  <div style="max-width: 1200px; margin: 0 auto; padding: 0 2rem 2rem;">
    <form method="GET" action="fleet.php" style="display: flex; flex-wrap: wrap; gap: 1rem; align-items: center;">
      ...
      <select name="sort" onchange="this.form.submit()"
        ...
        <option value="price_desc" <?php if($sort==='price_desc') echo 'selected'; ?>>Harga: Tinggi &rarr; Rendah</option>
        <option value="price_asc"  <?php if($sort==='price_asc')  echo 'selected'; ?>>Harga: Rendah &rarr; Tinggi</option>
        <option value="alpha"      <?php if($sort==='alpha')       echo 'selected'; ?>>A &rarr; Z</option>
      </select>
